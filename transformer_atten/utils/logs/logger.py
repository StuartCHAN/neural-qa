...
from transformer_atten.utils.logs import cloud_lib
...
def config_benchmark_logger(flag_obj=None):
  ...
  try:
    ...
    if (not hasattr(flag_obj, "benchmark_logger_type") or
      ...
      from transformer_atten.benchmark import benchmark_uploader as bu  # pylint: disable=g-import-not-at-top
...
class BenchmarkFileLogger(BaseBenchmarkLogger):
  ...
  def log_run_info(self, model_name, dataset_name, run_params, test_id=None):
    ...
    The schema of the run info follows transformer_atten/benchmark/datastore/schema.
