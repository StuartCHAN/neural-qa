...
def eva(question, lang):
    ''' 
    to get the command like: 
      curl -d "query=What is the capital of Germany?&lang=en" -X POST http://localhost:8080/gerbil
    '''
    ql = '\"query='+question+'&lang='+lang
    req = "-d %s -X POST http://localhost:8080/gerbil"%ql  
     
    tmpres = os.popen( req ) # however, the return value is a os._wrap_close object.
    res = tmpres.read() # I tried to decode it as a readable value lke string or list, but it returns a empty list.
    tmpres.close()
    return res ;

demo7 = eva(question,lang)
'''
