...
'''
class Relatives(Particle):
    
    Ex: "Who is the wife of Obama?"
    
    

    regex = (Lemmas("who be") + Pos("DT") + Person() +
             Pos("of") + Person())  + Question(Pos("."))

    def interpret(self, match):
        person  = IsPerson() + DirectorOf(match.Person)
        person_name = HasKeyword(person )
        return person_name, "literal" '''