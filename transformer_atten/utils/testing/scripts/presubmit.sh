...
# usage: models >: ./transformer_atten/utils/testing/scripts/presubmit.sh
# usage: models >: ./transformer_atten/utils/testing/scripts/presubmit.sh lint py2_test py3_test
set +x
...
lint() {
  ...
  for file in `find transformer_atten/ -name '*.py' ! -name '*test.py' -print`
  do
  ...
  for file in `find transformer_atten/ -name '*test.py' -print`
  do
    echo "Linting ${file}"
...
py_test() {
  ...
  for test_file in `find transformer_atten/ -name '*test.py' -print`
  do
